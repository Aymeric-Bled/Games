...
public class Pow_2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            document = database.getDocument("2048");
            if (document != null) {
                document.getArray("tab_number");
                document.getInt("score");
                hasSave = true;
            }
        ...
        if (hasSave) {
            for (int i = 0; i < 4; i++) {
                for (int j = 0; j < 4; j++) {
                    tab_number[i][j] = tab_number2[i][j] = document.getArray("tab_number").getArray(i).getInt(j);
                }
            }
            s = document.getInt("score");
        } else {
            for (int i = 0; i < 4; i++) {
                for (int j = 0; j < 4; j++) {
                    tab_number[i][j] = 0;
    ...
    protected void onDestroy() {
        ...
        MutableDocument document = new MutableDocument("2048");
        MutableArray array = new MutableArray();
        for (int i = 0 ; i < 4; i++){
            MutableArray arr = new MutableArray();
            for (int j = 0; j < 4; j++){
                arr.addInt(tab_number[i][j]);
            }
            array.addArray(arr);
        }
        document.setArray("tab_number", array);
        document.setInt("score", s);
        try {
